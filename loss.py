...
class SupConLoss(nn.Module):
    ...
    def forward(self, features, labels=None, mask=None):
        ...
        if len(features.shape) < 3:  # [batch_size, n_views, feature_dim]
...
class infoNCE(nn.Module):
    ...
    def reply_get_posAndneg(self, reply_label, source_centers):
        # get the label of q
        q_label = reply_label
        feature_pos = source_centers[q_label]
        negative_idx = []
        for i in range(self.class_num):
            if i != reply_label:
                negative_idx.append(i)
    ...
    def source_get_posAndneg(self, source_label, target_centers, re_org_label):
        q_label = source_label
        pos_samples, neg_samples = [], []

        match_indices = [i for i, label in enumerate(re_org_label) if label == q_label]
        if not match_indices:
            return pos_samples, neg_samples

        pos_samples = [target_centers[idx] for idx in match_indices]
        neg_indices = [i for i in range(len(target_centers)) if i not in match_indices]
        neg_samples = [target_centers[idx] for idx in random.sample(neg_indices, min(len(neg_indices), self.neg_samples))]
