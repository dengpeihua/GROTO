...
def arg_parser():
    ...
    parser.add_argument('--source_model', default='./model_source/20240521-2241source-free-OH_amazon_ce_singe_gpu_vit_B_16_imagenet1k_best.pkl')
    parser.add_argument('--source_centers', default='./model_source/20240521-2241source_centers_mean.pkl')
...
class reply_dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, images, labels, buffer_per_class, soft_predictions, return_id=False):
        ...
        # assert len(self.images) == len(self.labels) == len(self.batch_soft_pred), "Data lengths are not consistent!"
...
def obtain_label(total_sample_nums, source_weight, dataloader, model, confi_class_idx, args, epoch):
    ...
    feats1 = []
    feats2 = []
    prediction_bank = torch.zeros(1, 31).cuda()
...
class reply_buffer():  # memory bank
    ...
    def construct_exemplar_set(self, images, model):
        ...
        now_class_mean = np.zeros((1, 768))  # for vit, vit feature dimension is 768

        for i in range(self.m):
            # Calculate distance from each image to current mean
            x = class_mean - (now_class_mean + feature_extractor_output) / (i + 1)
            # Calculate Euclidean norm of each row vector
            x = np.linalg.norm(x, axis=1)
            # Select image closest to mean
            index = np.argmin(x)
            now_class_mean += feature_extractor_output[index]

            exemplar_indexes.append(index)  # Save prototype image index
            exemplar.append(images[index])  # Add prototype image to prototype image set
            soft_predar.append(buffer_output[index])  # Add prototype prediction soft label to prototype soft label set
            feas_past.append(feature_extractor_output[index])  # Add prototype normalized features to set
        
        # Update intra-class prototype memory bank and prototype soft label prediction memory bank
        self.exemplar_indexes.append(exemplar_indexes)  # Save prototype image index set
...
if __name__ == '__main__':
    ...
    source = args.source
    target = args.target
    logger = setup_logging('office31', my_dataset.domains[source], my_dataset.domains[target])
    ...
    # Start incremental learning loop
    for incre_idx in range(total_cls_nums // incre_cls_nums):
        ...
        # Load target domain training dataset
        ...
        # Load target domain test dataset
        ...
        if incre_idx == 0:  # First round uses pretrained ViT model
        ...
        else:  # Non-first rounds load model saved from previous stage
            net = load_full_model('./model_source/target_office31_{}_2_{}_vit_B_16_imagenet1k_DA_last_stage{}.pt'.format(source, target, incre_idx - 1), n_gpus)
        ...
        for epoch in range(n_epoches):
            ...
            for iter_idx in range(len(target_train_dl)):
                ...
                rep_loss = torch.tensor(0.).cuda()  # Initialize distillation loss
            ...
            torch.save(model, './model_source/target_office31_{}_2_{}_vit_B_16_imagenet1k_DA_last_stage{}.pt'.format(source, target, incre_idx))
