...
from source_datasets import FileListDataset2
...
class Trainer(object):
    ...
    def train(self):
        ...
        print(
            'imagenet1k_total' + time_stamp_launch + 'model : vit_B_16  lr: %s' % args.lr)
        ...
        source_file = imgNet_dataset.files[source]
        ...
        source_train_ds = FileListDataset2(list_path=source_file, path_prefix=imgNet_dataset.prefixes[source],
        ...
        for i in range(epochs):
            ...
            if i % 3 == 0:
                ...
                if acc >= best_acc:
                    best_acc = acc
                    best_model_path = './model_source/' + time_stamp_launch + '-single_gpu_imagenet1k_ce_vit_B_16_best.pkl'
                    torch.save(net, best_model_path)

            if i % 10 == 0:
                acc = val_net(net, val_loader)
                print("Epoch %d running_loss=%.3f, acc=%.3f" % (i + 1, avg_loss, acc))
                model_path = './model_source/' + time_stamp_launch + '-single_gpu_imagenet1k_ce_vit_B_16.pkl'
        ...
        torch.save(source_centers_mean, './model_source/' + time_stamp_launch + 'vit_B_16_imagenet1k_source_centers_mean.pkl')
