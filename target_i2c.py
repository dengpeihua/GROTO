...
def arg_parser():
    ...
    parser.add_argument('--source_model', default='./model_source/20240725-0923-single_gpu_imagenet1k_ce_vit_B_16.pkl')
    parser.add_argument('--source_centers', default='./model_source/20240725-0923vit_B_16_imagenet1k_source_centers_mean.pkl')
...
if __name__ == '__main__':
    ...
    imgNet_dataset = Imagenet_Dataset(
        ...
        prefix='./dataset/ImageNet-Caltech', is_join=False)
    source = 0
    target = 0
    logger = setup_logging('ImageNet-Caltech', imgNet_dataset.domains[source], cal_dataset.domains[target])
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    n_gpus = torch.cuda.device_count() if torch.cuda.is_available() else 0
    
    logger.info(f"Using {n_gpus} GPU(s)")
    logger.info(f"Source domain: {imgNet_dataset.domains[source]}, Target domain: {cal_dataset.domains[target]}")
    ...
    for incre_idx in range(len(cal_84_cls_list) // incre_cls_nums):
        ...
        if incre_idx == 0:
            ...
            net = load_full_model('./model_source/target_i2c_{}_2_{}_ViT_DA_last_stage{}.pt'.format(source, target, incre_idx - 1), n_gpus)
        ...
        for epoch in range(n_epoches):
            ...
            torch.save(net, './model_source/target_i2c_{}_2_{}_ViT_DA_last_stage{}.pt'.format(source, target, incre_idx))
    ...
    logger.info(f'From {imgNet_dataset.domains[source]} to {cal_dataset.domains[target]}, best accuracy of different stages:')
    logger.info(np.round(best_acc_stages, 3))
    logger.info(f'From {imgNet_dataset.domains[source]} to {cal_dataset.domains[target]}, last accuracy of different stages:')
    logger.info(np.round(last_acc_stages, 3))
    logger.info(f'From {imgNet_dataset.domains[source]} to {cal_dataset.domains[target]}, top_ten accuracy of different stages:')
