...
from target_datasets import FileListDataset
...
def arg_parser():
    ...
    parser.add_argument('--n_epoches', default=30, type=int)
    parser.add_argument('--source_model', default='./model_source/20240701-1714source-free-OH_Art_ce_singe_gpu_vit_B_16_imagenet1k_best.pkl')
    parser.add_argument('--source_centers', default='./model_source/20240701-1714source_centers_mean.pkl')
...
def obtain_label(total_sample_nums, source_weight, dataloader, model, confi_class_idx, args, epoch, logger):
    """
...
def get_confi_classes(source_model, target_data_loader, n_gpus, logger):
    """
...
def get_one_classes_imgs(target_train_loader, class_idx, confi_label_dict, total_sample_nums, aug_nums, logger):
    """
...
if __name__ == '__main__':
    ...
    source = args.source
    target = args.target
    logger = setup_logging('office-home', my_dataset.domains[source], my_dataset.domains[target])
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    n_gpus = torch.cuda.device_count() if torch.cuda.is_available() else 0
    
    logger.info(f"Using {n_gpus} GPU(s)")
    logger.info(f"Source domain: {my_dataset.domains[source]}, Target domain: {my_dataset.domains[target]}")
    ...
    diff_of_centers = torch.zeros((total_cls_nums, 768))
    ...
    for incre_idx in range(total_cls_nums // incre_cls_nums):
        logger.info(f"Starting incremental stage {incre_idx + 1}/{total_cls_nums // incre_cls_nums}")
        ...
        target_train_ds = FileListDataset(list_path=target_file, path_prefix=my_dataset.prefixes[target],
                                        return_id=True,
                                        transform=transform_train, filter=(lambda x: x in target_train_classes))
        ...
        confi_class_idx, confi_class_values, total_sample_nums = get_confi_classes(pretrained_net, target_train_dl, n_gpus, logger)
        ...
        if incre_idx == 0:
            ...
            net = load_full_model('./model_source/target_office-home_{}_2_{}_vit_B_16_imagenet1k_DA_last_stage{}.pt'.format(source, target, incre_idx - 1), n_gpus)
        ...
        for epoch in range(n_epoches):
            ...
            if epoch < 5:
                pred_label_dict, _, best_acc, confi_class_idx, class_centers, confi_class_probs = obtain_label(total_sample_nums, source_weight, target_train_dl, pretrained_net, confi_class_idx, args, epoch, logger)
            else:  # After warm-up stage, obtain pseudo labels with model
                pred_label_dict, _, best_acc, confi_class_idx, class_centers, confi_class_probs = obtain_label(total_sample_nums, source_weight, target_train_dl, model, confi_class_idx, args, epoch, logger)
                ...
                for confi_idx, confi_class in enumerate(confi_class_idx):
                    if this_stage_save_imgs:  # Update image prototypes
                        if confi_class not in confi_cls_history:
                            ...
                            imgs = get_one_classes_imgs(target_train_dl, confi_class, pred_label_dict, total_sample_nums, aug_nums, logger)
            ...
            acc_list = val_pclass(model, target_test_loader, total_cls_nums, total_cls_nums)
            ...
            _, source_only_acc, target_train_acc, _, _, _  = obtain_label(total_sample_nums, source_weight, target_train_dl, model, confi_class_idx, args, epoch, logger)
            ...
            torch.save(model, './model_source/target_office-home_{}_2_{}_vit_B_16_imagenet1k_DA_last_stage{}.pt'.format(source, target, incre_idx))
    ...
    logger.info(f'From {my_dataset.domains[source]} to {my_dataset.domains[target]}, best accuracy of different stages:')
    logger.info(np.round(best_acc_stages, 3))
    logger.info(f'From {my_dataset.domains[source]} to {my_dataset.domains[target]}, last accuracy of different stages:')
    logger.info(np.round(last_acc_stages, 3))
    logger.info(f'From {my_dataset.domains[source]} to {my_dataset.domains[target]}, top_ten accuracy of different stages:')
